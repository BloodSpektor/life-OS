...
import { Bell, Plus, Trash2, Apple, Pill, Droplets, Dumbbell, Moon, Utensils } from "lucide-react";
import { NotificationModal } from "@/components/NotificationModal";
import { useNutritionStore, Meal } from "@/lib/nutrition-store";
...
export default function RemindersPage() {
    ...
    const fetchMeals = async () => {
        ...
        try {
            ...
            console.error("Error fetching meals for reminders:", error);
        }
    };
    ...
    const handleCreateNotification = async (data: { category: string; title: string; time: string; days: string[]; comment?: string }) => {
        try {
    ...
    const handleUpdateNotification = async (data: { category: string; title: string; time: string; days: string[]; comment?: string }) => {
    ...
    const handleToggleEnabled = async (notification: Notification) => {
        try {
            const response = await fetch(`http://localhost:3002/api/notifications/${notification.id}`, {
                method: "PATCH",
                headers: getAuthHeaders(),
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Meals Section */}
            {nutritionStore.meals.length > 0 && (
                <Card>
                    <CardHeader>
                        <CardTitle className="text-sm font-medium capitalize">Приемы пищи сегодня</CardTitle>
                    </CardHeader>
                    <CardContent>
                                        ...
                                        <div className="flex items-center gap-4 flex-1">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-3 mt-1 text-sm text-gray-500">
                                                    ...
                                                    <span>•</span>
                                                    <span>
                                                        Б:{meal.protein}г У:{meal.carbs}г Ж:{meal.fats}г
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        >
                                            <div className="flex items-center gap-4 flex-1">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {notification.comment && <p className="text-sm text-gray-500 mt-1">{notification.comment}</p>}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <Switch checked={notification.isEnabled} onCheckedChange={() => handleToggleEnabled(notification)} />
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={(e) => {
